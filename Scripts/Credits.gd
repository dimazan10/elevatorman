...
func _start_story() -> void:
	_label.text = ""
	_sub_label.text = ""
	await _type_text(_label, Localization.t("credits_story"), 0.05)
	await get_tree().create_timer(3.0).timeout
	await _fade_label(_label)
	_phase = 1
	_label.text = ""
	_sub_label.text = ""
	await _type_text(_label, Localization.t("credits_thanks"), 0.07)
	await get_tree().create_timer(3.0).timeout
	await _fade_label(_label)
	_phase = 2
	_label.text = ""
	_sub_label.text = ""
	await _type_text(_label, Localization.t("credits_names"), 0.05)
